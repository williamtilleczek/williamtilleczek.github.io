---
title: "TEACHING"
about: 
  template: marquee
resources: 
  - "SOC204.pdf"
  - "HUM010.pdf"
format: 
      html:
        include-in-header: 
          - text: |
              <style>
                a {
                  color: #000000; /* Black for all links */
                }
              </style>
---

I take pride in challenging students to move beyond their limitations, while giving them the support they need to do so, all in an environment of mutual respect and collaboration.  One of my students stated my goal better than I could: “a subtle but ever-present mixture of high respect for his students in our current form and deep faith in their ability to improve/elevate our thinking.”

### **L'Université de Montréal (Instructor of Record)**

POL2090: *Pensée politique prémoderne* \[Premodern Political Thought\] (L'Université de Montréal), Winter 2025 and Winter 2026.

```{=html}
<p class="text-left">
  <a class="btn btn-secondary btn-sm" href="https://pol.umontreal.ca/public/FAS/science_politique/Documents/1-PROGRAMMES_ET_COURS/Cours_et_horaires/Hiver_2026/POL2090_H26_Tilleczek.pdf" target="_blank">
    </i>&ensp;2026 Syllabus here
  </a>
```

```{=html}
<p class="text-left">
  <a class="btn btn-secondary btn-sm" href="https://pol.umontreal.ca/public/FAS/science_politique/Documents/1-PROGRAMMES_ET_COURS/Cours_et_horaires/Hiver_2025/POL2090_H25_Tilleczek.pdf" target="_blank">
    </i>&ensp;2025 Syllabus here
  </a>
```

Pol 2050: Représentation politique \[Political representation\] (L'Université de Montréal), Fall 2025
```{=html}
<p class="text-left">
  <a class="btn btn-secondary btn-sm" href="https://pol.umontreal.ca/public/FAS/science_politique/Documents/1-PROGRAMMES_ET_COURS/Cours_et_horaires/Automne_2025/POL2050_Tilleczek.pdf" target="_blank">
    </i>&ensp;Syllabus here 
  </a>
```

### **Deep Springs College (Instructor of Record)**

SOC204: *Hannah Arendt and Simone Weil: Work and Politics* (Deep Springs College), Winter 2023.

```{=html}
<p class="text-left">
  <a class="btn btn-secondary btn-sm" href="SOC204.pdf" target="_blank">
    <i class="fa-solid fa-file-arrow-down"></i>&ensp;Syllabus here
  </a>
```


HUM010: *Introduction to Ancient Greek Language & Literature* (Deep Springs College), Winter 2023.

```{=html}
<p class="text-left">
  <a class="btn btn-secondary btn-sm" href="HUM010.pdf" target="_blank">
    <i class="fa-solid fa-file-arrow-down"></i>&ensp;Syllabus here
  </a>
```


### **Harvard University (Teaching Fellow for Professor Michael Sandel)**

GENED 1181: *Meritocracy and its Critics*, (Harvard University), Fall 2022.

GENED 1171: *Justice: Ethics in an Age of Pandemic and Racial Reckoning* (Harvard University), Fall 2020 (Assistant Head Teaching Fellow).

GOVT E-1045: *Justice* (Harvard Extension School), Spring 2019

ETHRSON 39: *Money, Markets, and Morals* (Harvard University), Fall 2018. 
